---
title: "GPGFrontend"
permalink: /software/misc/gpgfrontend/
excerpt: "Open-Source PGP Encryption"
---

GpgFrontend is a modern, easy-to-use, compact, cross-platform, and installation-free GnuPG front-end tool.
It visualizes most of the common OpenPGP operations such as signing and encryption.
It aims to allow ordinary users to quickly use GPG and make professional users more convenient.
GpgFrontend supports some new features of OpenPGP [^gpg].

Features:

* Open source, free, can be used with or without installation.
* Can run on Windows, Linux and macOS.
* Just double-click, and then you can use it freely.
* Before use, you only need to install gnupg. A tiny tool of a few MB.
* Supports multiple languages. Don’t forget to help me translate this software.

### Key Facts

* Developer/Publisher: [Saturneric](https://bktus.com/)
* License: Open Source (GNU GPL-3.0)
* Price: Free.
* Web:
  * [GitHub Repository](https://github.com/saturneric/GpgFrontend)
* Help: Help is provided in the README.md and by the developer via email
	* [README.md](https://github.com/saturneric/GpgFrontend/blob/main/README.md)
	* [Email](mailto:eric@bktus.com)
  
[^gpg]: GnuPG only supports OpenPGP up to [RFC4880+RFC5581+RFC6637](/about/gnupg) and newer versions generate incompatible formats by default
